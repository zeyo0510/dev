    ...
    internal static class ContextMenuHelper
    {
        ...
        public static string GetCommandStringA(IContextMenu iContextMenu, uint idcmd, bool executeString)
        {
            ...
            iContextMenu.GetCommandString(
                idcmd,
                (executeString ? WinAPI.GCS.VERBA : WinAPI.GCS.HELPTEXTA),
                0,
                bytes,
                WinAPI.MAX_PATH);
        ...
        public static void InvokeCommand(IContextMenu iContextMenu, uint cmd, string parentDir, Point ptInvoke)
        {
            ...
            invoke.fMask = WinAPI.CMIC.UNICODE | WinAPI.CMIC.PTINVOKE |
                ((Control.ModifierKeys & Keys.Control) != 0 ? WinAPI.CMIC.CONTROL_DOWN : 0) |
                ((Control.ModifierKeys & Keys.Shift) != 0 ? WinAPI.CMIC.SHIFT_DOWN : 0);
            ...
            out IContextMenu iContextMenu)
        {
            if (parent.GetUIObjectOf(
                        ...
                        (uint)pidls.Length,
                        pidls,
                        ref WinAPI.IID_IContextMenu,
                        IntPtr.Zero,
                        out iContextMenuPtr) == WinAPI.S_OK)
            {
                iContextMenu =
        ...
        public static bool GetNewContextMenu(ShellItem item, out IntPtr iContextMenuPtr, out IContextMenu iContextMenu)
        {
                    ...
                    out iContextMenuPtr) == WinAPI.S_OK)
            {
                ...
                if (Marshal.QueryInterface(
                    iContextMenuPtr,
                    ref WinAPI.IID_IShellExtInit,
                    out iShellExtInitPtr) == WinAPI.S_OK)
                {
