...
namespace ShellDll
{
  internal class StreamStorageProvider : IFileInfoProvider, IDirInfoProvider
  {
    ...
    public WinAPI.STATSTG GetFileInfo()
    {
      if (tempStreamReader != null)
    ...
    private WinAPI.STATSTG dirInfo;
    private bool dirInfoRetrieved = false;

    public WinAPI.STATSTG GetDirInfo()
    {
      if (dirInfoRetrieved)
        return dirInfo;
      else if (providerItem != null)
      {
          ...
          {
            tempStorage.Stat(out dirInfo, WinAPI.STATFLAG.NONAME);
  ...
  public class ShellStreamReader : Stream, IDisposable
  {
    ...
    private WinAPI.STATSTG streamInfo;
    ...
    {
      if (CanWrite) stream.SetSize(value);
    }
    ...
    internal WinAPI.STATSTG StreamInfo
    {
        get
        {
          if (!streamInfoRead)
          {
            stream.Stat(out streamInfo, WinAPI.STATFLAG.NONAME);
            streamInfoRead = true;
          }
          return streamInfo;
        }
    ...
    {
        WinAPI.STGM grfmode = WinAPI.STGM.SHARE_DENY_WRITE;
        
        switch (access)
        {
          case FileAccess.ReadWrite:
            grfmode |= WinAPI.STGM.READWRITE;
            break;

          case FileAccess.Write:
            grfmode |= WinAPI.STGM.WRITE;
            break;
        }

        if (parentStorage != null)
        {
          if (parentStorage.OpenStream(shellItem.Text + (shellItem.IsLink ? ".lnk" : string.Empty), IntPtr.Zero, grfmode, 0, out streamPtr) == WinAPI.S_OK)
          {
            stream = (IStream)Marshal.GetTypedObjectForIUnknown(streamPtr, typeof(IStream));
          }
          else if (access != FileAccess.Read)
          {
            if (parentStorage.OpenStream(shellItem.Text + (shellItem.IsLink ? ".lnk" : string.Empty), IntPtr.Zero, WinAPI.STGM.SHARE_DENY_WRITE, 0, out streamPtr) == WinAPI.S_OK)
            {
