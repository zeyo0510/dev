...
//hack to get the browser context menu to show up.
//$(".blocklyBlockWorkspace").attr("oncontextmenu", "return false;")
...
Blockly.Generator.prototype.workspaceToCodeNoImports = function(workspace) {
  var code = [];
  this.init(workspace);
  var blocks = workspace.getTopBlocks(true);

  blocks.sort(function(f,s){
      var f_id = f.getFieldValue("NAME") || f.getFieldValue("LIB") || f.getFieldValue("FUN") || f.getFieldValue("func_name")
      var s_id = s.getFieldValue("NAME") || s.getFieldValue("LIB") || s.getFieldValue("FUN") || s.getFieldValue("func_name")
  ...
  //gets rid of white space after or before {}(); chars
...
Blockly.Variables.allLocalVariables = function(workspace) {
  if(!workspace)
...
Blockly.Generator.prototype.workspaceToCodeAlpha = function(workspace) {
  if(!workspace)
    throw("must pass a workspace to workspaceToCodeAlpha")
...
Blockly.Procedures.flyoutCategory = function(blocks, gaps, margin, workspace) {  
 if (Blockly.Blocks['procedures_defnoreturn']) {
  ...
  function populateProcedures(procedureList, templateName) {
    // console.log("pros: ", procedureList)
    // console.log("templan: ", templateName)
  ...
  for(var key in imports){
    ...
    try{
      ...
      for(var i = 0; i < all_blocks.length; i++)
      {
        ...
        {
          if(current.attributes.disabled && current.attributes.disabled.value == "true") 
            continue;
      ...
      for(var j = 0; j < all_blocks.length; j++)
      {
        ...
        if(current.attributes["type"].value == "procedures_defnoreturn")
        {
          ...
          bool = false;
          proc_type = 'procedures_callnoreturn';
          
        } else if (current.attributes["type"].value == "procedures_defreturn"){

          bool = true;
          proc_type = 'procedures_callreturn';
          ...
          for(var i in current.children) {
            if(current.children[i].attributes["name"] && current.children[i].attributes["name"].value == "NAME") {
              short_name = current.children[i].innerHTML;
              break;
            }
          }
          
          //continue to check next block if the name of the function is not one that was exported
          if(exports.indexOf(short_name) == -1) {
            continue;
          }
          
          var param_names = []
          
          var mutations_node = null 
          for(var i in current.children) {
            if(!current.children[i].nodeName) continue;
            nodeSample = current.children[i];
            if(current.children[i].nodeName.toUpperCase() == "MUTATION") {
              mutations_node = current.children[i];
              break;
            }
          ...
          if(mutations_node != null){
            var param_nodes = mutations_node.children
            param_names = [];
            for(var k = 0; k < param_nodes.length; k++){
              param_names.push(param_nodes[k].attributes["name"].value);
            } 
          ...
          populateProcedures([[name, param_names, bool]], proc_type);
...
Blockly.Blocks['import'] = {
  init: function() {
    ...
    var initialLoad = setInterval(function(){
      var val = that.getFieldValue('LIB')
...
Blockly.JavaScript['texturepack'] = function(block) {
  ...
  var text_url_front = text_url.slice(0,7);
  var text_url_back = text_url.slice(7, text_url.length);
  if (text_url_front == "http://" && text_url_back){
      code = value_player + '.setResourcePack(\''+text_url+'\');\n';
  }
  else {
       var code = value_player+'.setResourcePack(\'http://mod.learntomod.com/editable_image_groups/'+text_url+'.zip\');\n';
  }
  return code;
};
...
Blockly.Blocks['instrument_bass_drum'] = {
  init: function() {
    ...
    this.appendValueInput("note")
        .setCheck("Tone")
        .appendField(new Blockly.FieldImage("http://thoughtstem.cms.dev.s3.amazonaws.com/bass_drum.png", 20, 20, "*"));
...
Blockly.Blocks['instrument_bass_guitar'] = {
  init: function() {
    ...
    this.appendValueInput("note")
        .setCheck("Tone")
        .appendField(new Blockly.FieldImage("http://thoughtstem.cms.dev.s3.amazonaws.com/bass_guitar.png", 20, 20, "*"));
    this.setOutput(true, "NoteSound");
    this.setTooltip('');
  }
};
...
Blockly.Blocks['instrument_piano'] = {
  init: function() {
    ...
    this.appendValueInput("note")
        .setCheck("Tone")
        .appendField(new Blockly.FieldImage("http://thoughtstem.cms.dev.s3.amazonaws.com/piano.png", 20, 20, "*"));
    this.setOutput(true, "NoteSound");
    this.setTooltip('');
  }
};
...
Blockly.Blocks['instrument_snare_drum'] = {
  init: function() {
    ...
    this.appendValueInput("note")
        .setCheck("Tone")
        .appendField(new Blockly.FieldImage("http://thoughtstem.cms.dev.s3.amazonaws.com/snare_drum.png", 20, 20, "*"));
    this.setOutput(true, "NoteSound");
    this.setTooltip('');
  }
};
...
Blockly.Blocks['instrument_sticks'] = {
  init: function() {
    ...
    this.appendValueInput("note")
        .setCheck("Tone")
        .appendField(new Blockly.FieldImage("http://thoughtstem.cms.dev.s3.amazonaws.com/sticks.png", 20, 20, "*"));
    this.setOutput(true, "NoteSound");
    this.setTooltip('');
  }
};
  ...
  init: function() {
    ...
    this.appendDummyInput()
        ...
        .appendField(new Blockly.FieldDropdown([["pig", "PIG"], 
            ...
            ["wither", "WITHER"], ["wolf", "WOLF"],["horse","HORSE"]]), "TYPE");
    this.setInputsInline(true);
...
Blockly.Blocks['npc_horse_color'] = {
  init: function() {
    this.setHelpUrl('http://www.example.com/');
    this.setColour(40);
    this.appendDummyInput()
        .appendField("Horse Color")
        .appendField(new Blockly.FieldDropdown([["white", "WHITE"], 
            ["creamy", "CREAMY"], ["chestnut", "CHESTNUT"], 
            ["brown", " BROWN"], ["black", "BLACK"],
            ["grey", "GRAY"], ["dark brown", "DARK_BROWN"]]), "TYPE");
    this.setInputsInline(true);
    this.setColour('240');
    this.setOutput(true, "Horse.Color");
    this.setTooltip('');
  }
};
Blockly.Blocks['npc_dye_color'] = {
  init: function() {
    this.setHelpUrl('http://www.example.com/');
    this.setColour(40);
    this.appendDummyInput()
        .appendField("Dye Color")
        .appendField(new Blockly.FieldDropdown([["white", "WHITE"],
            ["orange", "ORANGE"], ["magenta", "MAGENTA"],
            ["light blue", " LIGHT_BLUE"], ["yellow", "YELLOW"],
            ["lime", "LIME"], ["pink", "PINK"],["grey", "GREY"],
            ["silver", "SILVER"], ["cyan", "CYAN"],
            ["purple", "PURPLE"], ["blue", "BLUE"],
            ["brown", "BROWN"], ["green", "GREEN"],["red", "RED"],
            ["black", "BLACK"]]), "TYPE");
    this.setInputsInline(true);
    this.setColour('240');
    this.setOutput(true, "Horse.Color");
...
Blockly.Blocks['anon_func_with_return'] = {
  init: function() {
    this.setHelpUrl('http://www.example.com/');
    this.appendStatementInput("CODE")
        .appendField("function");
...
Blockly.JavaScript['anon_func_with_return'] = function(block) {
  var statements_code = Blockly.JavaScript.statementToCode(block, 'CODE');
...
Blockly.JavaScript['import'] = function(block) {
  var text_lib = block.getFieldValue('LIB');
    var xml = window.imports[text_lib].split("<SEP>")[0]
    var js =  window.imports[text_lib].split("<SEP>")[1]
...
Blockly.JavaScript['gui'] = function(block) {
  ...
  return "browser.loadURL(\"" + gui + "\");\n";
};

Blockly.Blocks['texture_pack_reference'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Texture Pack")
        .appendField(new Blockly.FieldTextInput("url"), "url");
    this.setInputsInline(true);
    this.setTooltip('');
    this.setPreviousStatement(true);
...
Blockly.JavaScript['texture_pack_reference'] = function(block) {
  var pack_url = block.getFieldValue('url');
  return "this.texture_pack = \""+pack_url +"\";\n";
};


Blockly.Blocks['schematic_paste'] = {
  init: function() {

    this.setColour(210);
    this.appendDummyInput()
        .appendField("Paste schematic")
        .appendField(new Blockly.FieldTextInput("nickname-name"), "schematic_name");
...
Blockly.JavaScript['schematic_paste'] = function(block) {
  var value_location = Blockly.JavaScript.valueToCode(block, 'location', Blockly.JavaScript.ORDER_ATOMIC);
  var value_player = Blockly.JavaScript.valueToCode(block, 'player', Blockly.JavaScript.ORDER_ATOMIC);
  var text_name = block.getFieldValue('schematic_name');
  // TODO: Assemble JavaScript into code variable.
  var code = "var schemData = JSON.parse(nova.util.SchematicHandler.fetchSchematic(\"" + text_name + "\"));\n";
  code += "var missingList = [];var missedCount = 0;\n";
  code += "schemData.forEach(function(block){\n";
  code += "\tvar v = " + value_location + ".add(new Vector3D(block.x, block.y, block.z));\n";
  code += "\tvar blockType = \"minecraft:\" + block.name.replace(/:/g, \"_\");\n";
  code += "\tif(blockManager.getFactory(blockType).isPresent()) {\n";
  code += "\t\tblockManager.getFactory(blockType).ifPresent(function(b){" + value_player + ".entity().world().setBlock(v, b);});\n";
  code += "\t} else {\n";
  code += "\t\tmissedCount++;\n";
  code += "\t\tif(missingList.indexOf(blockType) == -1) {missingList.push(blockType);}\n";
  code += "\t}\n";
  code += "})\n";
  code += "if(missingList.length > 0) {\n";
  code += "\tvar message = \"[\";\n";
  code += "\tmissingList.forEach(function(blockName){message+=blockName + \", \"});\n";
  code += "\tmessage += \"]\";\n";
  code += "\tmessage += \"There were \" + missedCount + \" blocks that had to be skipped, because nova did not recognize the names above.  Please report a screenshot of the list above to learntomod@thoughtstem.com\";\n"
  code += "\tnetworkManager.sendChat(" + value_player + ", message);\n"
  code += "}\n"
  return code;
...
Blockly.Blocks['npc_get_entity'] = {
  init: function() {
    this.setHelpUrl('http://www.example.com/');
    this.setColour(240);
    this.appendValueInput("npc")
        .appendField("Entity of");
    this.setOutput(true);
    this.setTooltip('');
  }
};

Blockly.JavaScript['npc_get_entity'] = function(block) {
  var value_npc = Blockly.JavaScript.valueToCode(block, 'npc', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = value_npc+'.getEntity()';
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};

Blockly.Blocks['npc_set_tamed'] = {
  init: function() {
    this.setHelpUrl('http://www.example.com/');
    this.setColour(240);
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.appendValueInput("npc")
        .appendField("NPC entity");
    this.appendValueInput("isTrue")
        .appendField("set tamed");
    this.setInputsInline(true);
    this.setTooltip('');
  }
};

Blockly.JavaScript['npc_set_tamed'] = function(block) {
  var value_npc = Blockly.JavaScript.valueToCode(block, 'npc', Blockly.JavaScript.ORDER_ATOMIC);
  var value_isTrue = Blockly.JavaScript.valueToCode(block, 'isTrue', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = value_npc+'.setTamed(' + value_isTrue+ ');  \n';
  return code;
};

Blockly.Blocks['npc_set_color'] = {
  init: function() {
    this.setHelpUrl('http://www.example.com/');
    this.setColour(240);
    this.setPreviousStatement(true);
...
Blockly.JavaScript['npc_set_color'] = function(block) {
  var value_npc = Blockly.JavaScript.valueToCode(block, 'npc', Blockly.JavaScript.ORDER_ATOMIC);
  var value_color = Blockly.JavaScript.valueToCode(block, 'color', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = value_npc+'.setColor(' + value_color + ');  \n';
  return code;
};

Blockly.Blocks['npc_get_inventory'] = {
  init: function() {
    this.setHelpUrl('http://www.example.com/');
    this.setColour(240);
    this.appendValueInput("npc")
        .appendField("inventory of");
    this.setOutput(true);
    this.setTooltip('');
  }
};

Blockly.JavaScript['npc_get_inventory'] = function(block) {
  var value_npc = Blockly.JavaScript.valueToCode(block, 'npc', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = value_npc+'.getInventory()';
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};

Blockly.Blocks['npc_set_armor'] = {
  init: function() {
    this.appendValueInput("npc")
        .appendField("NPC entity");
    this.appendValueInput("armor")
        .appendField("set armor");
    this.setInputsInline(true);
    this.setColour(240);
    this.setPreviousStatement(true);
    this.setNextStatement(true);
...
Blockly.JavaScript['npc_set_armor'] = function(block) {
  var value_npc = Blockly.JavaScript.valueToCode(block, 'npc', Blockly.JavaScript.ORDER_ATOMIC);
  var value_armor = Blockly.JavaScript.valueToCode(block, 'armor', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = value_npc+'.setArmor(new ItemStack('+value_armor+'));  \n';
  return code;
};


Blockly.Blocks['npc_set_saddle'] = {
  init: function() {
    this.appendValueInput("npc")
        .appendField("NPC entity");
    this.appendValueInput("saddle")
        .appendField("set saddle");
    this.setInputsInline(true);
    this.setColour(240);
    this.setPreviousStatement(true);
    this.setNextStatement(true);
...
Blockly.JavaScript['npc_set_saddle'] = function(block) {
  var value_npc = Blockly.JavaScript.valueToCode(block, 'npc', Blockly.JavaScript.ORDER_ATOMIC);
  var value_saddle = Blockly.JavaScript.valueToCode(block, 'saddle', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = value_npc+'.setSaddle(new ItemStack('+value_saddle+')); \n';
  return code;
};

Blockly.JavaScript['npc_horse_color'] = function(block) {
  var dropdown_type = block.getFieldValue('TYPE');
  // TODO: Assemble JavaScript into code variable.
  var code = 'org.bukkit.entity.Horse.Color.'+dropdown_type;
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};

Blockly.JavaScript['npc_dye_color'] = function(block) {
  var dropdown_type = block.getFieldValue('TYPE');
  // TODO: Assemble JavaScript into code variable.
  var code = 'org.bukkit.DyeColor.'+dropdown_type;
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};

Blockly.Blocks['npc_wander_goal'] = {
  init: function() {
    this.appendDummyInput()
	.appendField("Wander Goal");
    this.appendValueInput("npc")
        .appendField("NPC");
    this.appendValueInput("x")
        .appendField("distance x")
	.setCheck("Number");
    this.appendValueInput("y")
	.appendField("y")
	.setCheck("Number");
...
Blockly.JavaScript['npc_wander_goal'] = function(block) {
  var value_npc = Blockly.JavaScript.valueToCode(block, 'npc', Blockly.JavaScript.ORDER_ATOMIC);
  var value_x = Blockly.JavaScript.valueToCode(block, 'x', Blockly.JavaScript.ORDER_ATOMIC);
     var value_y = Blockly.JavaScript.valueToCode(block, 'y', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'net.citizensnpcs.api.ai.goals.WanderGoal.createWithNPCAndRange('+ value_npc +','+ value_x +','+ value_y +').shouldExecute(); \n';
  return code;
};


Blockly.Blocks['npc_isSpawned'] = {
  init: function() {
    this.appendValueInput("NPC")
	.appendField("is Spawned NPC");
    this.setInputsInline(true);
    this.setOutput(true);
    this.setTooltip('');
    this.setColour(240);
...
Blockly.JavaScript['npc_isSpawned'] = function(block) {
 var value_NPC = Blockly.JavaScript.valueToCode(block, 'NPC', Blockly.JavaScript.ORDER_ATOMIC);
  var code = value_NPC + ".isSpawned()\n";
  return [code, Blockly.JavaScript.ORDER_NONE];  
};
Blockly.Blocks['npc_getNPC'] = {
  init: function() {
    this.appendValueInput("NPC")
	.appendField("get NPC");
    this.setInputsInline(true);
...
Blockly.JavaScript['npc_getNPC'] = function(block) {
 var value_NPC = Blockly.JavaScript.valueToCode(block, 'NPC', Blockly.JavaScript.ORDER_ATOMIC);
  var code = value_NPC + ".getNPC() \n";
  return [code, Blockly.JavaScript.ORDER_NONE];  
};
Blockly.Blocks['npc_isNPC'] = {
  init: function() {
    this.appendValueInput("NPC")
	.appendField("is NPC");
    this.setInputsInline(true);
...
Blockly.JavaScript['npc_isNPC'] = function(block) {
 var value_NPC = Blockly.JavaScript.valueToCode(block, 'NPC', Blockly.JavaScript.ORDER_ATOMIC);
  var code = "net.citizensnpcs.api.CitizensAPI.getNPCRegistry().isNPC("+value_NPC+")\n";
  return [code, Blockly.JavaScript.ORDER_NONE];  
};
...
///////////////////////// FORGE /////////////////////////
//textures
Blockly.Blocks['addTexture'] = {
  init: function() {
    this.appendDummyInput()
      .appendField("Add Texture Pack:");
...
Blockly.JavaScript['addTexture'] = function(block) {
  var value_domain = Blockly.JavaScript.valueToCode(block, 'domain', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'RenderManager.createTexture('+value_domain+');\n';
  return code;
};

//items
Blockly.Blocks['createItem'] = {
  init: function() {
    this.appendDummyInput()
      .appendField("Create Item:");
    this.appendValueInput("domain")
      .setCheck("String")
      .appendField("Texture Pack Domain:");
    this.appendValueInput("name")
      .setCheck("String")
      .appendField("Texture Name:");
...
Blockly.JavaScript['createItem'] = function(block) {
  var value_domain = Blockly.JavaScript.valueToCode(block, 'domain', Blockly.JavaScript.ORDER_ATOMIC);
  var value_name = Blockly.JavaScript.valueToCode(block, 'name', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'ItemManager.createItem('+value_domain+', '+value_name+');\n';
  return code;
};

//Events
Blockly.Blocks['block_event'] = {
  init: function() {
    this.appendDummyInput()
        .appendField(new Blockly.FieldDropdown([["harvestBlock", "HARVESTBLOCK"], ["breakBlock", "BREAKBLOCK"], ["placeBlock", "PLACEBLOCK"], ["rightClick", "RIGHT_CLICK_BLOCK"], ["leftClick", "LEFT_CLICK_BLOCK"]]), "eventTypes")
    this.setOutput(true, "BlockEvent");
    this.setColour(210);
    this.setTooltip('');
    this.setHelpUrl('http://www.example.com/');
  }
...
Blockly.JavaScript['block_event'] = function(block) {
  var dropdown_eventtypes = block.getFieldValue('eventTypes');
  
  var code = '\''+dropdown_eventtypes+'\'';
                      
  return [code, Blockly.JavaScript.ORDER_NONE];
};
                      
Blockly.Blocks['explosion_client'] = {
  init: function() {
    this.appendValueInput("radius")
        .setCheck("Number")
        .appendField("create explosion with a radius of");
    this.setInputsInline(true);
    this.setPreviousStatement(true);
    this.setNextStatement(true);
    this.setColour(120);
...
Blockly.JavaScript['explosion_client'] = function(block) {
  var value_radius = Blockly.JavaScript.valueToCode(block, 'RADIUS', Blockly.JavaScript.ORDER_ATOMIC);
                      
  var code = 'event.world.createExplosion(null, event.pos.getX(), event.pos.getY(), event.pos.getZ(),';
  code += value_radius+', true);\n';
  return code;
};
...
Blockly.Blocks['event_handler'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("function")
        .appendField(new Blockly.FieldTextInput("event_handler"), "HANDLER")
        .appendField("event");
    this.appendStatementInput("event_actions")
        .setCheck("EventAction");
    this.setInputsInline(true);
    this.setOutput(true, "EventHandler");
    this.setColour(160);
    this.setTooltip('');
    this.setHelpUrl('http://www.example.com/');
  }
...
Blockly.JavaScript['event_handler'] = function(block) {
  var text_handler = block.getFieldValue('HANDLER');
  var statements_event_actions = Blockly.JavaScript.statementToCode(block, 'event_actions');
  
  var code = 'function '+text_handler+'(event) {\n';
  code += statements_event_actions + '}\n';
                      
  return [code, Blockly.JavaScript.ORDER_NONE];
};

//DRONE
Blockly.Blocks['new_drone_client'] = {
  init: function() {
    this.setHelpUrl('/book/docs/drone-docs');
    this.setColour(90);
    this.appendDummyInput()
...
Blockly.JavaScript['new_drone_client'] = function(block) {
  var code = 'new Drone(event.pos)';
  
  return [code, Blockly.JavaScript.ORDER_NONE];
};

Blockly.Blocks['drone_move_client'] = {
  init: function() {
    this.setHelpUrl('/book/docs/drone-docs');
    this.setColour(90);
    this.appendDummyInput()
...
Blockly.JavaScript['drone_move_client'] = function(block) {
  var variable_drone = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('DRONE'), Blockly.Variables.NAME_TYPE);
  var dropdown_direction = block.getFieldValue('DIRECTION');
  var text_distance = block.getFieldValue('DISTANCE');
...
Blockly.Blocks['drone_move_2_client'] = {
  init: function() {
    this.setHelpUrl('/book/docs/drone-docs');
    this.setColour(90);
    this.appendDummyInput()
...
Blockly.JavaScript['drone_move_2_client'] = function(block) {
  var variable_drone = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('DRONE'), Blockly.Variables.NAME_TYPE);
  var dropdown_direction = block.getFieldValue('DIRECTION');
  var value_distance = Blockly.JavaScript.valueToCode(block, 'DISTANCE', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
...
Blockly.Blocks['drone_rotate_client'] = {
  init: function() {
    this.setHelpUrl('/book/docs/drone-docs');
    this.setColour(90);
    this.appendDummyInput()
...
Blockly.JavaScript['drone_rotate_client'] = function(block) {
  var variable_drone = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('DRONE'), Blockly.Variables.NAME_TYPE);
  var dropdown_direction = block.getFieldValue('DIRECTION');
  var value_num_rotations = Blockly.JavaScript.valueToCode(block, 'NUM_ROTATIONS', Blockly.JavaScript.ORDER_ATOMIC) % 4;
  var dir = 1;
  if(dropdown_direction == "counter-clockwise")
    dir = -1;
  // TODO: Assemble JavaScript into code variable.
  var code = variable_drone + ".turn("+dir+", "+value_num_rotations+");\n";
  return code;
};

Blockly.Blocks['block_place_client'] = {
  init: function() {
    this.setHelpUrl('http://www.example.com/');
    this.setColour(90);
    this.appendDummyInput()
...
Blockly.JavaScript['block_place_client'] = function(block) {
  var value_material = Blockly.JavaScript.valueToCode(block, 'MATERIAL', Blockly.JavaScript.ORDER_ATOMIC);
  var variable_drone = Blockly.JavaScript.variableDB_.getName(block.getFieldValue('DRONE'), Blockly.Variables.NAME_TYPE);
  // TODO: Assemble JavaScript into code variable.
  var code = variable_drone + ".place(" + value_material + ");\n";
  return code;
};

Blockly.Blocks['addevent_blockspecific'] = {
  init: function() {
...
Blockly.JavaScript['addevent_blockspecific'] = function(block) {
  var value_handler = Blockly.JavaScript.valueToCode(block, 'handler', Blockly.JavaScript.ORDER_ATOMIC);
  var value_type = Blockly.JavaScript.valueToCode(block, 'type', Blockly.JavaScript.ORDER_ATOMIC);
  var value_block = Blockly.JavaScript.valueToCode(block, 'block', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'EventManager.addEvent(\n'+value_handler+', \n'+value_type+', \n'+value_block+'\n);\n';
  return code;
};
...
Blockly.Blocks['create_item_pickaxe'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Create Pickaxe");
    this.appendValueInput("domain")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Texture Pack Domain");
    ...
    this.appendValueInput("category")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Category");
    this.appendValueInput("materialID")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Material Type");
    this.setInputsInline(false);
...
Blockly.JavaScript['create_item_pickaxe'] = function(block) {
  var value_domain = Blockly.JavaScript.valueToCode(block, 'domain', Blockly.JavaScript.ORDER_ATOMIC);
  var value_itemid = Blockly.JavaScript.valueToCode(block, 'itemID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_category = Blockly.JavaScript.valueToCode(block, 'category', Blockly.JavaScript.ORDER_ATOMIC);
  var value_materialid = Blockly.JavaScript.valueToCode(block, 'materialID', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'ItemPickaxeManager.createItemPickaxe('+value_domain+', '+value_itemid+', '+(value_category ? value_category : null)+', '+(value_materialid ? value_materialid : null)+');\n';
  return code;
};

Blockly.Blocks['create_item_axe'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Create Axe");
    this.appendValueInput("domainID")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Texture Pack Domain");
    this.appendValueInput("itemID")
...
Blockly.JavaScript['create_item_axe'] = function(block) {
  var value_domainid = Blockly.JavaScript.valueToCode(block, 'domainID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_itemid = Blockly.JavaScript.valueToCode(block, 'itemID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_category = Blockly.JavaScript.valueToCode(block, 'category', Blockly.JavaScript.ORDER_ATOMIC);
  var value_materialid = Blockly.JavaScript.valueToCode(block, 'materialID', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'ItemAxeManager.createItemAxe('+value_domainid+', '+value_itemid+', '+(value_category ? value_category : null)+', '+(value_materialid ? value_materialid : null)+');\n';
  return code;
};


Blockly.Blocks['create_item_sword'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Create Sword");
    this.appendValueInput("domainID")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Texture Pack Domain");
...
Blockly.JavaScript['create_item_sword'] = function(block) {
  var value_domainid = Blockly.JavaScript.valueToCode(block, 'domainID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_itemid = Blockly.JavaScript.valueToCode(block, 'itemID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_category = Blockly.JavaScript.valueToCode(block, 'category', Blockly.JavaScript.ORDER_ATOMIC);
  var value_materialid = Blockly.JavaScript.valueToCode(block, 'materialID', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'ItemSwordManager.createItemSword('+value_domainid+', '+value_itemid+', '+(value_category ? value_category : null)+', '+(value_materialid ? value_materialid : null)+');\n';
  return code;
};


Blockly.Blocks['create_item_shovel'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Create Shovel");
    this.appendValueInput("domainID")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Texture Pack Domain");
...
Blockly.JavaScript['create_item_shovel'] = function(block) {
  var value_domainid = Blockly.JavaScript.valueToCode(block, 'domainID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_itemid = Blockly.JavaScript.valueToCode(block, 'itemID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_category = Blockly.JavaScript.valueToCode(block, 'category', Blockly.JavaScript.ORDER_ATOMIC);
  var value_materialid = Blockly.JavaScript.valueToCode(block, 'materialID', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'ItemShovelManager.createItemShovel('+value_domainid+', '+value_itemid+', '+(value_category ? value_category : null)+', '+(value_materialid ? value_materialid : null)+');\n';
  return code;
};


Blockly.Blocks['create_item_hoe'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Create Hoe");
    this.appendValueInput("domainID")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Texture Pack Domain");
...
Blockly.JavaScript['create_item_hoe'] = function(block) {
  var value_domainid = Blockly.JavaScript.valueToCode(block, 'domainID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_itemid = Blockly.JavaScript.valueToCode(block, 'itemID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_category = Blockly.JavaScript.valueToCode(block, 'category', Blockly.JavaScript.ORDER_ATOMIC);
  var value_materialid = Blockly.JavaScript.valueToCode(block, 'materialID', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'ItemHoeManager.createItemHoe('+value_domainid+', '+value_itemid+', '+(value_category ? value_category : null)+', '+(value_materialid ? value_materialid : null)+');\n';
  return code;
};

Blockly.Blocks['create_item_armor'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Create Armor");
    this.appendValueInput("domain")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Texture Pack Domain");
    this.appendValueInput("itemID")
        ...
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Item Name");
    this.appendValueInput("armorType")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
    ...
    this.appendValueInput("material")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Texture Layer");
    this.appendValueInput("durability")
        .setCheck("Number")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Durability");
    this.appendValueInput("damageReduction")
        .setCheck("Array")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Damage Reduction");
    this.appendValueInput("enchantability")
        .setCheck("Number")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Enchantability");
    this.appendValueInput("category")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Category");
...
Blockly.JavaScript['create_item_armor'] = function(block) {
  var value_domain = Blockly.JavaScript.valueToCode(block, 'domain', Blockly.JavaScript.ORDER_ATOMIC);
  var value_itemid = Blockly.JavaScript.valueToCode(block, 'itemID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_armortype = Blockly.JavaScript.valueToCode(block, 'armorType', Blockly.JavaScript.ORDER_ATOMIC);
  var value_material = Blockly.JavaScript.valueToCode(block, 'material', Blockly.JavaScript.ORDER_ATOMIC);
  var value_durability = Blockly.JavaScript.valueToCode(block, 'durability', Blockly.JavaScript.ORDER_ATOMIC);
  var value_damagereduction = Blockly.JavaScript.valueToCode(block, 'damageReduction', Blockly.JavaScript.ORDER_ATOMIC);
  var value_enchantability = Blockly.JavaScript.valueToCode(block, 'enchantability', Blockly.JavaScript.ORDER_ATOMIC);
  var value_category = Blockly.JavaScript.valueToCode(block, 'category', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'ItemArmorManager.createItemArmor('+value_domain+', '+value_itemid+', '+value_armortype+', '+(value_material ? value_material : null)+', '+(value_durability ? value_durability : null)+', '+(value_damagereduction ? value_damagereduction : null)+', '+(value_enchantability ? value_enchantability : null)+', '+(value_category ? value_category : null)+');\n';
  return code;
};
...
Blockly.JavaScript['create_item'] = function(block) {
  var value_domain = Blockly.JavaScript.valueToCode(block, 'domain', Blockly.JavaScript.ORDER_ATOMIC);
  var value_itemid = Blockly.JavaScript.valueToCode(block, 'itemID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_category = Blockly.JavaScript.valueToCode(block, 'category', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'ItemManager.createItem('+value_domain+', '+value_itemid+', '+value_category+');\n';
  return code;
};
...
Blockly.JavaScript['create_block'] = function(block) {
  var value_domain = Blockly.JavaScript.valueToCode(block, 'domain', Blockly.JavaScript.ORDER_ATOMIC);
  var value_itemid = Blockly.JavaScript.valueToCode(block, 'itemID', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'BlockManager.createBlock('+value_domain+', '+value_itemid+');\n';
  return code;
};
...
Blockly.Blocks['create_block_cmh'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Create Block");
    this.appendValueInput("domain")
    ...
    this.appendValueInput("materialData")
        .setCheck("MaterialData")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Material Data");
    this.appendValueInput("harvestData")
        .setCheck("HarvestData")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Harvest Data");
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setColour(210);
    this.setTooltip('');
...
Blockly.JavaScript['create_block_cmh'] = function(block) {
  var value_domain = Blockly.JavaScript.valueToCode(block, 'domain', Blockly.JavaScript.ORDER_ATOMIC);
  var value_itemid = Blockly.JavaScript.valueToCode(block, 'itemID', Blockly.JavaScript.ORDER_ATOMIC);
  var value_category = Blockly.JavaScript.valueToCode(block, 'category', Blockly.JavaScript.ORDER_ATOMIC);
  var value_materialdata = Blockly.JavaScript.valueToCode(block, 'materialData', Blockly.JavaScript.ORDER_ATOMIC);
  var value_harvestdata = Blockly.JavaScript.valueToCode(block, 'harvestData', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'BlockManager.createBlock('+value_domain+', '+value_itemid+', '+(value_category ? value_category : null)+', '+(value_materialdata ? value_materialdata : null)+', '+(value_harvestdata ? value_harvestdata : null)+');\n';
  return code;
};



Blockly.Blocks['create_harvest_data'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Harvest Data");
    this.appendValueInput("dropItem")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Drop Item");
    this.setOutput(true, "HarvestData");
    this.setColour(195);
    this.setTooltip('');
    this.setHelpUrl('http://www.example.com/');
  }
...
Blockly.JavaScript['create_harvest_data'] = function(block) {
  var value_dropitem = Blockly.JavaScript.valueToCode(block, 'dropItem', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'HarvestData.create('+value_dropitem+')';
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};
...
Blockly.Blocks['create_harvest_data_quantity'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Harvest Data");
    this.appendValueInput("dropItem")
        ...
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Drop Item");
    this.appendValueInput("min_quantity")
        .setCheck("Number")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Min Quantity");
    this.appendValueInput("max_quantity")
        .setCheck("Number")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Max Quantity");
    this.setOutput(true, "HarvestData");
    this.setColour(195);
    this.setTooltip('');
    this.setHelpUrl('http://www.example.com/');
...
Blockly.JavaScript['create_harvest_data_quantity'] = function(block) {
  var value_dropitem = Blockly.JavaScript.valueToCode(block, 'dropItem', Blockly.JavaScript.ORDER_ATOMIC);
  var value_min_quantity = Blockly.JavaScript.valueToCode(block, 'min_quantity', Blockly.JavaScript.ORDER_ATOMIC);
  var value_max_quantity = Blockly.JavaScript.valueToCode(block, 'max_quantity', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'HarvestData.create('+value_dropitem+', '+(value_min_quantity ? value_min_quantity : 1)+', '+(value_max_quantity ? value_max_quantity : 1)+')';
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};
...
Blockly.Blocks['create_harvest_data_all'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Harvest Data");
    this.appendValueInput("dropItem")
    ...
    this.appendValueInput("min_quantity")
        .setCheck("Number")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Min Quantity");
    this.appendValueInput("max_quantity")
        .setCheck("Number")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Max Quantity");    
    this.appendValueInput("harvestTool")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Harvest Tool");
    this.appendValueInput("harvestLevel")
        .setCheck("Number")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Harvest Level");
    this.appendValueInput("hardness")
        .setCheck("Number")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("(optional) Hardness");
    this.setOutput(true, "HarvestData");
    this.setColour(195);
    this.setTooltip('');
    this.setHelpUrl('http://www.example.com/');
...
Blockly.JavaScript['create_harvest_data_all'] = function(block) {
  var value_dropitem = Blockly.JavaScript.valueToCode(block, 'dropItem', Blockly.JavaScript.ORDER_ATOMIC);
  var value_min_quantity = Blockly.JavaScript.valueToCode(block, 'min_quantity', Blockly.JavaScript.ORDER_ATOMIC);
  var value_max_quantity = Blockly.JavaScript.valueToCode(block, 'max_quantity', Blockly.JavaScript.ORDER_ATOMIC);
  var value_harvesttool = Blockly.JavaScript.valueToCode(block, 'harvestTool', Blockly.JavaScript.ORDER_ATOMIC);
  var value_harvestlevel = Blockly.JavaScript.valueToCode(block, 'harvestLevel', Blockly.JavaScript.ORDER_ATOMIC);
  var value_hardness = Blockly.JavaScript.valueToCode(block, 'hardness', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  //var code = 'HarvestData.create('+value_dropitem+', '+(value_quantity ? value_quantity : 1)+', '+(value_harvesttool ? value_harvesttool : null)+', '+(value_harvestlevel ? value_harvestlevel : 2)+', '+(value_hardness ? value_hardness : 1.5)+')';
  var code = 'HarvestData.create('+value_dropitem+', '+(value_harvesttool ? value_harvesttool : null)+', '+(value_harvestlevel ? value_harvestlevel : 2)+', '+(value_hardness ? value_hardness : 1.5)+', '+(value_min_quantity ? value_min_quantity : 1)+', '+(value_max_quantity ? value_max_quantity : 1)+')';
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};
...
Blockly.Blocks['create_material_data'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Material Data");
    this.appendValueInput("baseMaterialName")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Base Material");
    this.setOutput(true, "MaterialData");
    this.setColour(195);
    this.setTooltip('');
    this.setHelpUrl('http://www.example.com/');
  }
...
Blockly.JavaScript['create_material_data'] = function(block) {
  var value_basematerialname = Blockly.JavaScript.valueToCode(block, 'baseMaterialName', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'MaterialData.create('+value_basematerialname+')';
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};


Blockly.Blocks['create_material_data_all'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Material Data");
    this.appendValueInput("encouragement")
        .setCheck("Number")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Encouragement");
    this.appendValueInput("mobile")
        .setCheck("Number")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Mobile");
    this.appendValueInput("mapColor")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Map Color");
    this.appendDummyInput()
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Tool Required")
        .appendField(new Blockly.FieldCheckbox("FALSE"), "toolRequired")
        .appendField("Replaceable")
        .appendField(new Blockly.FieldCheckbox("FALSE"), "replaceable");
    this.appendDummyInput()
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Blocks Light")
        .appendField(new Blockly.FieldCheckbox("FALSE"), "blocksLight")
        .appendField("Flammable")
        .appendField(new Blockly.FieldCheckbox("FALSE"), "flammable");
    this.appendDummyInput()
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Solid")
        .appendField(new Blockly.FieldCheckbox("FALSE"), "solid")
        .appendField("Translucent")
        .appendField(new Blockly.FieldCheckbox("FALSE"), "translucent");
    this.setInputsInline(false);
    this.setOutput(true, "MaterialData");
    this.setColour(195);
    this.setTooltip('');
...
Blockly.JavaScript['create_material_data_all'] = function(block) {
  var value_encouragement = Blockly.JavaScript.valueToCode(block, 'encouragement', Blockly.JavaScript.ORDER_ATOMIC);
  var value_mobile = Blockly.JavaScript.valueToCode(block, 'mobile', Blockly.JavaScript.ORDER_ATOMIC);
  var value_mapcolor = Blockly.JavaScript.valueToCode(block, 'mapColor', Blockly.JavaScript.ORDER_ATOMIC);
  var checkbox_toolrequired = block.getFieldValue('toolRequired') == 'TRUE';
  var checkbox_replaceable = block.getFieldValue('replaceable') == 'TRUE';
  var checkbox_blockslight = block.getFieldValue('blocksLight') == 'TRUE';
  var checkbox_flammable = block.getFieldValue('flammable') == 'TRUE';
  var checkbox_solid = block.getFieldValue('solid') == 'TRUE';
  var checkbox_translucent = block.getFieldValue('translucent') == 'TRUE';
  // TODO: Assemble JavaScript into code variable.
  var code = 'MaterialData.create('+checkbox_flammable+','+value_encouragement+','+checkbox_replaceable+','+checkbox_translucent+','+checkbox_solid+','+value_mapcolor+','+checkbox_toolrequired+','+value_mobile+','+checkbox_blockslight+')';
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};

Blockly.Blocks['block_base_material_ac'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Base Material (A - C):");
    this.appendDummyInput()
        .appendField(new Blockly.FieldDropdown([
		["AIR", "air"], ["ANVIL", "anvil"], 
		["BARRIER", "barrier"], ["CACTUS", "cactus"], 
		["CAKE", "cake"], ["CARPET", "carpet"], 
		["CIRCUITS", "circuits"], ["CLAY", "clay"], 
		["CLOTH", "cloth"], ["CORAL", "coral"], 
		["CRAFTED_SNOW", "craftedSnow"]]), 
		"baseMaterial");
    this.setInputsInline(true);
...
Blockly.JavaScript['block_base_material_ac'] = function(block) {
  var dropdown_basematerial = block.getFieldValue('baseMaterial');
  // TODO: Assemble JavaScript into code variable.
  var code = '\''+dropdown_basematerial+'\'';
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};

Blockly.Blocks['block_base_material_dl'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Base Material (D - L):");
    this.appendDummyInput()
        .appendField(new Blockly.FieldDropdown([
		["DRAGON_EGG", "dragonEgg"], ["FIRE", "fire"], 
		["GLASS", "glass"], ["GOURD", "gourd"], 
		["GRASS", "grass"], ["GROUND", "ground"], 
		["ICE", "ice"], ["IRON", "iron"], 
		["LAVA", "lava"], ["LEAVES", "leaves"]]), 
		"baseMaterial");
    this.setInputsInline(true);
    this.setOutput(true, "String");
    this.setColour(260);
...
Blockly.JavaScript['block_base_material_dl'] = function(block) {
  var dropdown_basematerial = block.getFieldValue('baseMaterial');
  // TODO: Assemble JavaScript into code variable.
  var code = '\''+dropdown_basematerial+'\'';
  // TODO: Change ORDER_NONE to the correct strength.
...
Blockly.Blocks['block_base_material_pz'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Base Material (P - Z):");
    this.appendDummyInput()
        .appendField(new Blockly.FieldDropdown([
		["PACKED_ICE", "packedIce"], ["PISTON", "piston"], 
		["PLANTS", "plants"], ["PORTAL", "portal"], 
		["REDSTONE_LIGHT", "redstoneLight"], 
		["ROCK", "rock"], ["SAND", "sand"], 
		["SNOW", "snow"], ["SPONGE", "sponge"], 
		["TNT", "tnt"], ["VINE", "vine"], 
		["WATER", "water"], ["WEB", "web"], 
		["WOOD", "wood"]]), 
		"baseMaterial");
...
Blockly.JavaScript['block_base_material_pz'] = function(block) {
  var dropdown_basematerial = block.getFieldValue('baseMaterial');
  // TODO: Assemble JavaScript into code variable.
  var code = '\''+dropdown_basematerial+'\'';
  // TODO: Change ORDER_NONE to the correct strength.
...
Blockly.Blocks['block_map_color'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Map Color");
    this.appendDummyInput()
        .appendField(new Blockly.FieldDropdown([["AIR", "airColor"], ["CLAY", "clayColor"], ["CLOTH", "clothColor"], ["DIRT", "dirtColor"], ["FOLIAGE", "foliageColor"], ["GRASS", "grassColor"], ["ICE", "iceColor"], ["IRON", "ironColor"], ["SAND", "sandColor"], ["SNOW", "snowColor"], ["STONE", "stoneColor"], ["TNT", "tntColor"], ["WATER", "waterColor"], ["WOOD", "woodColor"]]), "mapColor");
    this.setInputsInline(true);
    this.setOutput(true, "MapColor");
    this.setColour(180);
    this.setTooltip('');
    this.setHelpUrl('http://www.example.com/');
  }
...
Blockly.JavaScript['block_map_color'] = function(block) {
  var dropdown_mapcolor = block.getFieldValue('mapColor');
  // TODO: Assemble JavaScript into code variable.
  var code = '\''+dropdown_mapcolor+'\'';
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};


Blockly.Blocks['material_type'] = {
  init: function() {
    this.appendDummyInput()
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Material Type")
        .appendField(new Blockly.FieldDropdown([["WOOD", "wood"], ["STONE", "stone"], ["IRON", "iron"], ["GOLD", "gold"], ["DIAMOND", "diamond"]]), "type");
    this.setInputsInline(true);
    this.setOutput(true, "String");
    this.setColour(180);
    this.setTooltip('');
...
Blockly.JavaScript['material_type'] = function(block) {
  var dropdown_type = block.getFieldValue('type');
  // TODO: Assemble JavaScript into code variable.
...
Blockly.Blocks['create_category_image'] = {
  init: function() {
    this.appendDummyInput()
        .appendField("Set Category Image:");
    this.appendValueInput("category_name")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Category Name");
    this.appendValueInput("category_image")
        .setCheck("String")
        .setAlign(Blockly.ALIGN_RIGHT)
        .appendField("Category Image");
    this.setInputsInline(true);
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setTooltip('');
...
Blockly.JavaScript['create_category_image'] = function(block) {
  var value_category_name = Blockly.JavaScript.valueToCode(block, 'category_name', Blockly.JavaScript.ORDER_ATOMIC);
  var value_category_image = Blockly.JavaScript.valueToCode(block, 'category_image', Blockly.JavaScript.ORDER_ATOMIC);
  // TODO: Assemble JavaScript into code variable.
  var code = 'RenderManager.setCategoryImage('+value_category_name+', '+value_category_image+');\n';
  return code;
};


Blockly.Blocks['category_names'] = {
  init: function() {
    this.appendDummyInput()
        .appendField(" Creative Tab")
        .appendField(new Blockly.FieldDropdown([["BUILDING_BLOCKS", "buildingBlocks"], 
        	["DECORATIONS", "decorations"], ["REDSTONE", "redstone"], 
        	["TRANSPORTATION", "transportation"], ["MISC", "misc"], 
        	["FOOD", "food"], ["TOOLS", "tools"], ["COMBAT", "combat"], 
        	["BREWING", "brewing"], ["MATERIALS", "materials"]]), "creative_tab");
    this.setInputsInline(false);
    this.setOutput(true, "String");
    this.setColour(195);
    this.setTooltip('');
...
Blockly.JavaScript['category_names'] = function(block) {
  var dropdown_creative_tab = block.getFieldValue('creative_tab');
  // TODO: Assemble JavaScript into code variable.
  var code = '\''+dropdown_creative_tab+'\'';
  // TODO: Change ORDER_NONE to the correct strength.
  return [code, Blockly.JavaScript.ORDER_NONE];
};
