...
namespace App.Windows.XPMinesweeper.Core
{
  public sealed partial class Mine
  {
    ...
    private int markedMineCount
    {
      ...
      {
        int i = 0;
        if (markedMine(Mine00)) i++;
        if (markedMine(Mine01)) i++;
        if (markedMine(Mine02)) i++;
        if (markedMine(Mine10)) i++;
        if (markedMine(Mine12)) i++;
        if (markedMine(Mine20)) i++;
        if (markedMine(Mine21)) i++;
        if (markedMine(Mine22)) i++;
        return i;
      }
    }
  }
